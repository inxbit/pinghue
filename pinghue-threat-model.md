...
- Release tags and the `pypi` environment are protected in the hosted repository using the checked-in ruleset templates; live state was verified on 2026-05-31.
...
| Website deploy workflow | Push to `main` touching `docs/` | `docs/` content -> GitHub Pages at pinghue.com | Deploys the static site with pinned actions; `pages: write`/`id-token: write` are scoped to the deploy job; a contract test validates the site before upload; the site is fully static with self-hosted fonts and no external scripts. | `.github/workflows/pages.yml:1`, `tests/site_pages.test.mjs:1`, `docs/CNAME:1` |
...
| TM-006 | Supply-chain attacker | Release tag workflow can publish artifacts. | Abuse mutable workflow dependencies, weak tag controls, or overbroad credentials to ship malicious packages. | Compromised PyPI/GitHub release artifacts. | Release artifacts, repository release controls | Pinned action SHAs, non-persisted checkout credentials, split publish job, OIDC, attestations, concurrency; branch/tag ruleset templates require pull requests, signatures, and protected tags (approving review count is 0 — solo maintainer; raise to 1 with self-review prevented once a second maintainer exists); live repository rulesets and the `pypi` environment were verified on 2026-05-31; `scripts/check-github-hardening.sh` validates hosted ruleset internals against `.github/repo-settings`, and `.github/workflows/repository-hardening.yml` checks hosted drift. | Hosted settings can still be changed outside Git, but scheduled drift checks make that visible. | Keep repository-hardening workflow visible and re-run manually after each repository migration; keep Dependabot action updates reviewed. | GitHub ruleset audit, scheduled drift workflow, release attestation verification, protected-environment approval logs. | Low | High | Medium |
...
| TM-008 | Supply-chain attacker or hosted-infrastructure compromise | The Pages workflow deploys `docs/` to pinghue.com on every push to `main`; the domain's DNS is managed outside the repository. | Alter site content or install commands via a malicious `docs/` change, workflow tampering, or DNS/custom-domain takeover, so visitors copy a poisoned install command. | Visitors install an attacker-chosen package or follow malicious guidance. | Installation guidance on pinghue.com, project reputation | `main` ruleset requires pull requests and signed commits; `pages.yml` actions are SHA-pinned with deploy permissions scoped to the deploy job; the site is static, self-hosted, and script-reviewed (no external resources) with a self-only CSP meta on both pages; `tests/site_pages.test.mjs` gates deploys, pins the exact copy-button install commands, and rejects non-local scripts. | DNS/registrar state for pinghue.com is outside repository controls. | Keep the site static and dependency-free; re-verify custom-domain and DNS state in the release checklist (added). | Pages deployment history; periodic external fetch of pinghue.com comparing install commands; DNS provider audit logs. | Low | Medium | Low |
...
| `docs/` + `.github/workflows/pages.yml` | Static-site content and the workflow that publishes it to pinghue.com. | TM-008 |

## Quality check

- Entry points covered: CLI args, host file, DNS resolution, ICMP/TCP sockets, no-TUI/TUI output, JSON export, doctor command, dependency audit, publish workflow, and website deploy workflow.
...
- Open assumptions: future host-file trust level and any future privileged wrapper remain unknown; hosted repository ruleset and PyPI environment state were verified on 2026-05-31 and should be re-verified after migrations or manual settings changes.
