...
- Release tags and the `pypi` environment are intended to be protected in the hosted repository using the checked-in ruleset templates.

Open questions:

- Whether the hosted GitHub repository already enforces the checked-in branch, tag, and `pypi` environment controls.
...
| --- | --- | --- | --- | --- |
| CLI arguments | Local operator command line | Operator input -> parser | Numeric, timing, port, duration, count, concurrency, and fail threshold values are validated. | `src/pinghue/cli.py:44`, `src/pinghue/cli.py:143` |
| Host file | `-f/--file` | Local file -> parser | Requires regular file, 1 MiB maximum, 5,000-line maximum. | `src/pinghue/hostfile.py:9` |
| DNS resolver | Hostnames in target list | Target text -> OS resolver | `--numeric` can avoid DNS and requires IP literals. | `src/pinghue/probes.py:53`, `src/pinghue/cli.py:126` |
...
| JSON export | `--output PATH` | Probe result -> local file | Target/error/host fields are sanitized and written through temp-file replace. | `src/pinghue/export.py:24`, `src/pinghue/export.py:79` |
| Doctor DNS probe | `--check --resolve-name` | Operator diagnostic host -> OS resolver | Used only for local diagnostics. | `src/pinghue/doctor.py:97`, `src/pinghue/cli.py:191` |
...
4. Report truncation during interruption:
   - Operator interrupts a maintenance-window run while JSON evidence is being written.
   - A partial file could masquerade as final evidence.
   - Current control: no-TUI signal handling records interruption, and JSON output is written to a temporary file before replace.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TM-001 | Semi-trusted host list provider | Operator copies target names from shared notes or generated inventory. | Inject terminal control characters into target names or error strings that later render in terminal output or reports. | Misleading operator evidence or hidden output lines. | Operator terminal integrity, JSON evidence reports | `sanitize_display` escapes C0/C1 controls (`src/pinghue/display.py:6`); no-TUI, TUI, and export call it (`src/pinghue/runner.py:184`, `src/pinghue/ui.py:217`, `src/pinghue/export.py:24`). | Residual risk from non-control Unicode confusables remains a documentation concern, not terminal control execution. | Keep display/export tests and treat any new output path as a required sanitizer caller. | Tests containing ESC/CSI payloads; manual review of new Rich/Textual output paths. | Low | Medium | Low |
...
| TM-003 | Local operator mistake or interrupted process | Operator selects an output path or interrupts during report writing. | Overwrite a chosen local path or leave partial JSON evidence. | Incomplete or misleading maintenance evidence. | JSON evidence reports | Temp-file replace and cleanup (`src/pinghue/export.py:79`); no-TUI interruption path records `exit_reason="interrupted"` (`src/pinghue/runner.py:210`). | Output path overwrite remains operator-controlled behavior. | Keep clear documentation that `--output` overwrites the selected path; avoid adding privileged execution. | JSON schema validation in tests; operational check for `exit_reason`. | Low | Medium | Low |
...
- Open assumptions: hosted repository ruleset and PyPI environment state remain outside the local tree and should be verified in GitHub settings.
